...
<script lang="ts">
    ...
    // Get desserts from menu items
    let dessertOptions: MenuItem[] = [];
    
    // Selected desserts (multiple possible)
    let selectedDesserts: Set<number> = new Set();
    ...
    function toggleDessert(dessertId: number) {
      if (selectedDesserts.has(dessertId)) {
        selectedDesserts.delete(dessertId);
      } else {
        selectedDesserts.add(dessertId);
      }
      // Trigger reactivity
  ...
  <div class="flex flex-col min-h-screen bg-orange-50">
    ...
    <div class="flex justify-between items-center p-4">
      <button class="text-orange-500 flex items-center" on:click={() => goto('/combo-selection')}>
    ...
    <div class="container mx-auto px-4 py-8">
      ...
      <div class="bg-white rounded-xl p-4 mb-8 shadow-md">
        ...
        <div class="flex justify-between text-gray-700">
          ...
          <span>Subtotal: ${getTotal(cartItems)}</span>
        </div>
      </div>
      ...
      <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {#each dessertOptions as dessert}
          <button 
            ...
            on:click={() => toggleDessert(dessert.id)}
          >
            <div class="relative">
