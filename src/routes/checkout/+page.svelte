...
<script lang="ts">
  ...
  import { cart, clearCart, getTotal } from "$lib/stores/cart";
  ...
  let orderNumber = Math.floor(Math.random() * 900) + 100; // Random 3-digit number
...
<div class="flex flex-col min-h-screen bg-orange-50">
  ...
  <div class="flex justify-between items-center p-4">
    <button class="text-orange-500 flex items-center" on:click={handleBack}>
    ...
    <div class="flex flex-col items-center px-4 py-8">
      ...
      <div class="grid grid-cols-3 gap-4 w-full max-w-md mb-10">
        <button 
          ...
          on:click={() => selectPaymentMethod('qr')}
        >
      ...
      <div class="w-full max-w-md">
        ...
        <div class="bg-white rounded-xl p-4 mb-6 shadow">
          ...
          <div class="flex justify-between items-center pt-3 border-t border-gray-200">
            ...
            <span class="font-bold text-lg">$ {getTotal(cartItems)}</span>
          </div>
        </div>
        
        <!-- Action Buttons -->
        <div class="flex justify-between">
          <button class="px-6 py-3 bg-orange-100 text-orange-500 rounded-full font-medium" on:click={() => goto('/menu')}>
            Continue Shopping
          </button>
          
          <button 
            class="px-8 py-3 bg-orange-500 text-white rounded-full font-medium"
            on:click={handlePayment}
            disabled={!selectedPayment || cartItems.length === 0}
            class:opacity-50={!selectedPayment || cartItems.length === 0}
          >
            Checkout
          </button>
        </div>
