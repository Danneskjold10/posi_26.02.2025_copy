...
<script lang="ts">
  ...
  let mainItem = $state<CartItem | null>(null);
  let selectedSide = $state<MenuItem | null>(null);
  let selectedDrink = $state<MenuItem | null>(null);
  
  // Get sides and drinks from menu items
  let sidesOptions = $state<MenuItem[]>([]);
  let drinksOptions = $state<MenuItem[]>([]);
  ...
  $effect(() => {
      ...
      // Get dining option from local storage
      ...
      if (cartItems.length > 0 && !mainItem) {
          mainItem = cartItems[cartItems.length - 1];
      }
  });
  ...
  function selectSide(side: MenuItem): void {
      selectedSide = side;
  }
  
  function selectDrink(drink: MenuItem): void {
      selectedDrink = drink;
  }
  
  function skipCombo(): void {
      // Skip combo selection and go to desserts page
      goto('/dessert-selection');
  }
  
  function proceedWithCombo(): void {
      ...
      if (selectedDrink) {
          ...
          // Add drink to cart
          cartItems.push(discountedDrink);
      }
  ...
  // Handlers for side selection
  function createSideSelectHandler(side: MenuItem) {
      return function() {
          selectSide(side);
      };
  }
  
  // Handlers for drink selection
  function createDrinkSelectHandler(drink: MenuItem) {
      return function() {
          selectDrink(drink);
      };
  }
              ...
              <div class="space-y-3 max-h-64 overflow-y-auto">
                  {#each sidesOptions as side}
                      <button 
                          ...
                          onclick={createSideSelectHandler(side)}
                      >
              ...
              <div class="space-y-3 max-h-64 overflow-y-auto">
                  {#each drinksOptions as drink}
                      <button 
                          ...
                          onclick={createDrinkSelectHandler(drink)}
                      >
