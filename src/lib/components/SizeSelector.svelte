...
<script lang="ts">
    ...
    // Props
    export let sizes: string[] = ["SM", "MD", "LG", "XL", "2XL"];
    export let selectedSize: number = 2;
    export let moveForward: () => void;
    
    // Carousel parameters - copied from your code
    const perspectiveFactor: number = 2;
    const translationFactor: number = 1.1;
    const carouselArcDeg: number = 90;
    
    let carousel3dLen: number = sizes.length;
  ...
  <div class="w-full h-full relative flex flex-col justify-center items-center bg-pink-100 pt-12 pb-16">
    <!-- Carousel container - moved down and made smaller -->
    ...
    <div class="w-full px-8 pb-4 relative mt-auto">
      ...
      <!-- Carousel Labels (Extended to cover all sizes) -->
