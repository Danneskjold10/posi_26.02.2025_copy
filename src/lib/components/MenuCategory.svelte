...
<script lang="ts">
  ...
  // Define props correctly using Svelte 5 syntax
  ...
  // Derived values
  const requiresCustomization = $derived(isCustomizableItem(item.id));
  ...
  function handleAddCustomizedItem(event: CustomEvent<AddToCartEventDetail>): void {
      const { item: customizedItem, customizations } = event.detail;
      
      // Define the item with customizations correctly
      const itemWithUpdatedDescription = {
          ...customizedItem,
          description: `${customizedItem.description} (Customized)`
      };
      
      // Add item with customizations to cart
      // Pass customizations as a separate parameter
      addToCart(itemWithUpdatedDescription, 1, customizations);
