<!-- src/lib/components/DirectEditableOrderItem.svelte -->
<script lang="ts">
    import { createEventDispatcher } from 'svelte';
    import type { CartItem } from '../stores/direct-cart';
    import { formatCustomizations, isCustomizableItem } from '../stores/direct-cart';
  ...
  <div class="flex items-center py-4 border-b border-gray-200 last:border-b-0">
    ...
    <div class="flex flex-col items-center ml-4">
      <div class="flex items-center space-x-2 border border-gray-300 rounded-lg overflow-hidden">
        ...
        <!-- Add an ID for direct DOM manipulation -->
        <span id="item-{index}-quantity" class="px-2 font-medium">{item.quantity}</span>
      ...
      <!-- Add an ID for direct DOM manipulation -->
      <div id="item-{index}-total" class="text-sm font-bold mt-2">
